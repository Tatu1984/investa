...
export const POST = withRateLimit(RULES.LOGIN, async (req: NextRequest, ctx) => {
  const input = LoginSchema.parse(await req.json());
  const result = await authService.login(input, { userAgent: ctx.userAgent, ipAddress: ctx.ipAddress });
